...
$error = '';
if (empty($_POST['wysylkaForm'])) {
    $error .= 'Należy wybrać sposób wysyłki';
}
...
if ($row['total'] < 1) {
    $message='Należy dodać produkty do koszyka' ;
    empty($error) ? $error=$message : $error .="<br>$message" ;
    $_SESSION['errorAddZamowienia']=$error;
    header('Location: ../MainPage/mainPage.php?cart=show');
    die();
} $id_klienta=$_SESSION['id']; $platnosc=$_POST['platnoscForm']; $wysylka=$_POST['wysylkaForm']; $data=date('Y-m-d'); $query="INSERT INTO zamowienia (Id_klienta, Rodzaj_platnosci, Usluga_wysylki, Data_zamowienia) VALUES ('$id_klienta','$platnosc', '$wysylka', '$data')" ; if (!$connection->query($query)) {
