<?php
    ...
    if ($_POST['akcja'] == 'insert') {
        ...
        if (empty($_POST['autor'])) {
            $errMsg .= '<br>Nie podano autora książki';
        }
        if (empty($_POST['opis'])) {
            $errMsg .= '<br>Nie podano opisu';
        }
        if (empty($_POST['gatunek'])) {
            $errMsg .= '<br>Nie podano gatunku';
        }
        if (empty($_POST['data'])) {
            $errMsg .= '<br>Nie podano daty wydania';
        }
        if (empty($_POST['wydawnictwo'])) {
            $errMsg .= '<br>Nie podano wydawnictwa';
        }
        if (empty($_POST['ocena'])) {
            $errMsg .= '<br>Nie podano oceny';
        }
        if (empty($_POST['cena'])) {
            $errMsg .= '<br>Nie podano ceny';
        }
